...
| 配置描述 | 版本 | 失败现象 | 原因分析 |
|----------|------|----------|----------|
| 纯 DPO 训练（无真实数据混合），`dpo_max_weight=0.8` | v1.5 | 3D 相似度与基模型无差异 | DPO 权重过高侵蚀去噪能力，且无真实数据维持基础能力 |
| 纯 DPO 训练，`dpo_max_weight=0.3`, `beta=0.3` | v1.6 | Valid% 59%→20%，SA 恶化 | 即使保守配置，无真实数据混合仍导致灾难性遗忘 |
...
| v1.7 | 5+ (进行中) | 4.153 (R3) | 47% | 63 | 混合训练已生效，有效率下降速度与 v1.6 同期相当 |
| v1.8 | — | — | — | — | 待运行 |
