...
## 2026-04-02 全局诊断：168 Epoch DPO 训练轨迹分析与行动计划

### 当前运行状态

| 项目 | 状态 |
|------|------|
| **DPO 训练 PID** | 1106426（Mar 30 启动，`DPO1_0_triSim.py params_x1x3x4_dpo_finetune_nps 0`） |
| **当前进度** | Epoch 168，第 56 轮 DPO 重采样 |
| **GPU 占用** | GPU 0 (RTX 4090) 16GB；GPU 1-2 (RTX 3090) 几乎空闲 |
| **评估进程** | `eval_unified.py` PID 2596332 同步运行中 |
| **训练结论** | ⚠️ **已失效**——有效率 6.2%，偏好对仅 3 对，应立即停止 |

### 完整训练轨迹（56 轮 DPO，/33/ 目录）

> 数据来源：`training/jobs/33/x1x3x4_dpo_finetune_nps/dpo_round_metrics.json`

| 阶段 | Round | Epoch | Valid% | Pairs | W_Total | 特征 |
|------|-------|-------|--------|-------|---------|------|
| **初始健康期** | 0-2 | 0-6 | 57-59% | 97-119 | 4.0-4.1 | 有效率最高，偏好对充足 |
| **快速衰退期** | 3-6 | 9-18 | 32-52% | 34-90 | 4.0-4.1 | 有效率开始崩塌 |
| **崩塌稳态期** | 7-55 | 21-165 | **6-21%** | **1-16** | 3.6-4.3 | 偏好对长期不足，DPO 信号无效 |

**关键节点**：
- **Round 1-2 (Epoch 3-6)**：有效率峰值 59.4%，偏好对峰值 119 对 — **这是最佳回滚点**
- **Round 6 (Epoch 18)**：有效率首次跌破 40%（32.3%）→ 如果保护性停训机制生效，应在此停止
- **Round 7 (Epoch 21)**：有效率 20%，偏好对 12 → 进入不可恢复的恶性循环
- **Round 51 (Epoch 153)**：有效率 6.2%，偏好对 **0** → 首次出现完全无偏好对的轮次
...
```python
'protect_stop_validity_rate': 0.40,       # 连续低于 40% 则停训
'protect_stop_patience_rounds': 2,         # 连续 2 轮
'protect_stop_min_pairs': 30,              # pair 数过低时立即停训
'buffer_gate_min_validity_rate': 0.40,     # 低于 40% 不写入 buffer
'buffer_gate_min_pairs': 50,               # 至少 50 对才覆盖旧数据
```

但 metrics JSON 中 **全部 56 轮的 `buffer_gate_passed` 和 `protective_stop_reason` 均为 N/A**，证明：
- 当前运行的进程（Mar 30 启动）使用的代码版本**未包含**这些机制的实际执行逻辑
- 或者这些字段在 `_collect_and_save_metrics()` 中未被传入

**行动项**：重启训练前必须验证这些机制实际工作。

### 核心矛盾：DPO 提高有效率但未提升 3D 相似度

以 Epoch 89 checkpoint 做的独立评估（750 样本）结果：

| 指标 | DPO (epoch 89) | OriginShepherd | SPD 基模型 | DPO vs SPD |
|------|----------------|----------------|------------|------------|
| **有效率** | **51.6%** ✅ | 38.3% | 48.5% | +3.1% |
| **Surf Sim** | 0.491 | **0.615** | 0.490 | +0.001 |
| **ESP Sim** | 0.244 | **0.342** | 0.247 | -0.003 |
| **Pharm Sim** | 0.238 | **0.303** | 0.239 | -0.001 |
| **SA Score** | 5.07 | 5.00 | 5.10 | -0.03 |

**结论**：DPO 56 轮训练（168 epoch）后，3D 相似度指标与 SPD 基模型**几乎完全相同**。DPO 偏好信号未能有效传导到 3D 形状/性质生成质量上。

### 根因分析

| # | 根因 | 影响 | 修复方向 |
|---|------|------|----------|
| 1 | **数据集极小**（仅 3 个 NPs，实际仅用 1 个训练 DPO） | 偏好对多样性严重不足 | 扩展到全部 3 个分子 |
| 2 | **DPO 权重过早过高** + 保护机制未生效 | 有效率崩塌不可逆 | 降低 `dpo_max_weight`，延长 ramp-up，确认 protect_stop 生效 |
| 3 | **评分公式与评估目标不匹配** | DPO 优化 total_score（混合分），评估单看每个维度 | 调整评分权重或分维度优化 |
| 4 | **固定原子数 70** | 参考分子原子数各异，尺寸不匹配影响 3D 对齐 | 可变原子数采样 |
| 5 | **x4 DPO 优化关闭**（`dpo_optimize_x4=False`） | 药效团模态无偏好梯度 | 考虑启用 |

### 行动计划（详见 Plan 文件）

1. **紧急**：停止 PID 1106426，释放 GPU
2. **快速出结果**：用 `epoch-epoch=009.ckpt`（Round 1-2，有效率 59%）做 4500 样本采样 + 三模型对比评估
3. **修复重启**：修正超参数（`dpo_max_weight` 0.15→0.05，`ramp_up` 10→30，`real_data_ratio` 0.7→0.85），确认保护机制生效后重启
...
| `dpo_max_weight` | **0.05** | `training` | DPO loss 最大权重，95%给标准去噪（v2.1: 0.15→0.05） |
| `dpo_ramp_up_epochs` | **30** | `training` | 达到最大 DPO 权重的 epochs 数（v2.1: 10→30） |
| `dpo_min_score_gap` | **0.10** | `training` | 偏好对最小分差阈值（v2.1: 0.15→0.10） |
...
| `num_samples_per_molecule` | **32** | `sampling` | 每个参考分子的采样数（v2.1: 16→32） |
...
| `dpo_max_weight=0.15`, `real_data_ratio=0.7`, `beta=0.3`, 168 epoch | v2.0 | Valid% 59%→6%，56轮后 Pairs≤1-7，保护机制未生效 | 权重仍偏高，`protect_stop`/`buffer_gate` 未实际执行，导致 48 轮无效训练浪费算力 |
...
| dpo_max_weight | 0.8 | 0.3 | 0.3 | 0.3 | 0.3 | **0.15** |
| dpo_ramp_up_epochs | 3 | 10 | 10 | 10 | 10 | 10 |
| real_data_ratio | — | — | 0.5 | 0.5 | 0.5 | **0.7** |
| iterative_dpo_enabled | True | True | True | **False** | **False** | **False** |
| x4 DPO Loss | 无 | 有 | 有 | 有 | 有 | 有（但 `dpo_optimize_x4=False`） |
| SA 惩罚权重 | ×0.5 | ×1.5 | ×1.5 | ×1.5 | ×1.5 | ×1.5 |
| 推理离散修复 | ✗ | ✗ | ✗ | ✗ | **✓** | **✓** |
| partial_denoise | ✗ | ✗ | ✗ | ✗ | ✗ | ✗（配置中有但未启用） |
| protect_stop | ✗ | ✗ | ✗ | ✗ | ✗ | 配置了但未生效 |
...
| Round | v1.6 | v1.7 | v1.9 | **v2.0** |
|-------|------|------|------|------|
| 0 | 3.928 | 3.952 | 4.016 | **3.999** |
| 1 | 4.061 | 4.039 | 4.134 | **4.051** |
| 2 | 4.098 | 4.041 | 4.120 | **4.029** |
| 3 | 4.147 | 4.153 | 3.938 | **4.140** |
| 4 | 4.067 | 4.027 | 4.076 | **4.056** |
...
| Round | v1.6 | v1.7 | v1.9 | **v2.0** |
|-------|------|------|------|------|
| 0 | 0.426 | 0.387 | 0.458 | **0.504** |
| 1 | 0.467 | 0.462 | 0.541 | **0.575** |
| 2 | 0.623 | 0.530 | 0.618 | **0.535** |
| 3 | 0.502 | 0.492 | 0.535 | **0.667** |
| 4 | 0.498 | 0.475 | 0.692 | **0.501** |
...
| v2.0 | 0 (56 轮) | Iterative DPO 禁用，168 epoch 未更新一次 |
...
| **v2.0** | **56** | **4.261 (R29)** | **6%** | **3** | 最长运行的版本（168 epoch）。前 2 轮有效率健康（59%），Round 6 后崩塌至 6-20%。保护性停训机制配置了但未生效，导致 48 轮无效训练。独立评估确认 3D 相似度未提升。 |
...
| 版本 | 日期 | 标签 | 状态 | 简要描述 |
|------|------|------|------|----------|
| v2.1 | 2026-04-02 | `[超参数]` | **待运行** | 修复超参数: `dpo_max_weight` 0.15→0.05, `ramp_up` 10→30, `real_data_ratio` 0.7→0.85, `samples` 16→32, `score_gap` 0.15→0.10 |
| v2.0 | 2026-03-30 | `[架构]` `[采样策略]` | **已失败**（168 epoch，Valid% 6%） | DPO 训练 `dpo_max_weight=0.15`，`real_data_ratio=0.7`。前 2 轮健康，R6 后崩塌，保护机制未生效导致 48 轮无效训练 |
