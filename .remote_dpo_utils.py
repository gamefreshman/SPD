...
_sa_score_dir = os.path.join(os.path.dirname(Chem.__file__), "..", "Contrib", "SA_Score")
...
try:
    ...
    _sascorer = None
    logging.getLogger(__name__).warning("sascorer not available; SA Score will be recorded as None")
...
def _edge_list_to_adjacency_matrix(num_atoms, edge_types):
    adjacency_matrix = np.zeros((num_atoms, num_atoms), dtype=int)
    edge_idx = 0
    for i in range(num_atoms):
        for j in range(i + 1, num_atoms):
            adjacency_matrix[i, j] = int(edge_types[edge_idx])
            edge_idx += 1
    return adjacency_matrix


def _build_3d_mol_from_arrays(atom_type_array, bond_adjacent_array, positions_3d):
    ...
    for i in range(n_atoms):
        for j in range(i + 1, n_atoms):
            if i not in atom_idx_mapping or j not in atom_idx_mapping:
                continue
            bond_type = bond_type_map.get(int(bond_adjacent_array[i][j]))
...
def _create_rdkit_molecule_from_sample(sample):
    x1 = sample.get("x1", {})
    atoms = np.asarray(x1.get("atoms", []))
    positions = np.asarray(x1.get("positions", []))
    bonds = x1.get("bonds")
    if atoms.size == 0 or positions.size == 0:
        return None
    if bonds is None:
        return create_rdkit_molecule(sample)
    expected_edges = len(atoms) * (len(atoms) - 1) // 2
    ...
    if not valid_atoms:
        return None
    original_bond_adjacent_array = _edge_list_to_adjacency_matrix(len(atoms), bonds)
    ...
    return _build_3d_mol_from_arrays(
...
class ShepherdScorer:
    ...
    def score_sample(self, sample, reference_bundle):
        ...
        raw_mol = _create_rdkit_molecule_from_sample(sample)
        if raw_mol is None:
            return None, {"is_valid": False, "reason": "mol_reconstruction_failed"}

        scoring_mol, sanitize_error = self.sanitize_scoring_molecule(raw_mol)
        if scoring_mol is None:
        ...
        try:
            logp = float(Crippen.MolLogP(scoring_mol))
        except Exception:
            logp = 2.5
        ...
        if _sascorer is not None:
            try:
                sa_score = float(_sascorer.calculateScore(Chem.RemoveHs(scoring_mol)))
                sa_fallback_used = False
            except Exception:
                logger.warning("SA Score calculation failed for a molecule; recording as None")

        sa_score_for_total = sa_score if sa_score is not None else 5.0
        sa_normalized = (sa_score_for_total - 1.0) / 9.0
        total_score = (
        ...
        scores = {
            "is_valid": True,
            "sa_score": sa_score,
            "sa_fallback_used": sa_fallback_used,
            "logp": logp,
        ...
        for group_id, samples in grouped_samples.items():
            ...
            for winner in winners:
                ...
                for loser in losers:
                    ...
                    winner_mol = _create_rdkit_molecule_from_sample(winner["sample"])
                    loser_mol = _create_rdkit_molecule_from_sample(loser["sample"])
