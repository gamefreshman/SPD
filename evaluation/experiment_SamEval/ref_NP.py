...
def run_sampling_on_gpu(gpu_id, tasks, molecule_index, molblock, charges_list, 
    ...
    # 处理分配给此GPU的所有任务
    for task_idx, (n_atoms, num_batches) in enumerate(tasks):
        print(f"GPU {gpu_id}: 处理任务 {task_idx + 1}/{len(tasks)}, n_atoms={n_atoms}, 批次数={num_batches}")
        
        task_results = []
        for batch_idx in range(num_batches):
            print(f"GPU {gpu_id}: n_atoms={n_atoms}, 批次 {batch_idx + 1}/{num_batches}")
...
if __name__ == '__main__':
    ...
    for mol_index in range(3):  # 0, 1, 2
        ...
        # 任务格式: (n_atoms, num_batches)
        all_tasks = [(n_atoms, num_batches_per_n_atoms) for n_atoms in N_ATOMS_LIST]
        ...
        for n_atoms in N_ATOMS_LIST:
            ...
            # 保存SDF文件（每个n_atoms一个文件）
            sdf_path = f'data/molecule_{mol_index}_n{n_atoms}_samples.sdf'
            ...
            for sample_data in samples:
                ...
                try:
                    sample_dict = {
                        'x1': {
                            'atoms': sample_data['atoms'],
                            'positions': sample_data['positions']
                        }
                    }
                    rdkit_mol = create_rdkit_molecule(sample_dict)
            ...
            print(f"  n_atoms={n_atoms}: {len(serializable_samples)} 个样本 -> {sdf_path}")
