...
distributions = np.load('../../data/conformers/distributions/atom_pharm_count.npz')

generated_samples = []
for n in range(num_batches):
    
    prob = distributions['gdb'][n_atoms,:]
    if prob.sum() > 0.0:
        prob = prob / prob.sum()
        N_x4 = int(np.random.multinomial(1, pvals=prob).argmax())
    else:
        prob = distributions['gdb'].sum(axis = 0)
        prob = prob / prob.sum()
        N_x4 = int(np.random.multinomial(1, pvals=prob).argmax())
...
with open(f"samples/GDB_unconditional/x1x4/samples_{file_index}.pickle", "wb") as f:
