...
with open('../../data/conformers/pdb/molblock_charges_bestdocked_lowestenergy.pkl', 'rb') as f:
...
for k in range(len(molblocks_and_charges)):
    ...
    n_atoms_list = list(N_atoms + np.arange(-12, 12)) # 25 options, centered around N_atoms
    n_atoms_list = n_atoms_list + n_atoms_list # 2 batches per choice of n_atoms
    ...
    with open(f"samples/PDB_analogues_bestdocked_lowestenergy/samples_{k}.pickle", "wb") as f:
